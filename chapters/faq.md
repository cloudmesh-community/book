...
`[text](url)` or `<url>` work

Under no circumstances must you use the `<` and `>` sign in markdown
if they are not related to urls. as they otherwise indicate html code,
which we like to avoid and lead to issues.
...
The ebooks have a ./bib dir. IN this you find a lot of references in
bibtex format (see Scientific Writing II). Bibtex can be easily
managed with emacs or jabref. There are other tools also.
