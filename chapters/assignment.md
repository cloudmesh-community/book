...
You are also allowed if you chose the *Cloudmesh Version 4* project to
...
: A project is the major activity that you chose as part of your
  ...
  *project report* and project code.  You will create a significant
  ...
  be build on top of a previous project but must have significant
  additions or modifications. If a previous project is used a detailed
  discussion is to be held of what has been improved. We will discuss
...
: A project report is to be delivered and continuously improved
  ...
  of the Architecture and code, with **benchmark** and demonstrated
  ...
  project and run it. Remember tables and figures do not count towards
  ...
  Please note that as we no longer use the ACM format but use
  ...
  * <https://github.com/cloudmesh-community/proceedings-fa18/tree/master/project-report>
  ...
  A single page has about 1000 in ACM format. References are managed in
...
: Data is to be hosted on IUs google drive if needed. If you have
  ...
  program that downloads the data either from the Web or IUs data
  storage.

Work Breakdown:

: This is an appendix to the document that describes in bullet form
   who did what in the project. This section comes in a new page
   ...
   that documents the statistics to demonstrate not only one student
...
: All bibliography has to be provided in a bibtex file that **MUST**
  ...
  takes some time so you want to do this early adn throughout the
  semester. What takes you less than 5 minutes a week, would quickly
  ...
  not lead to properly formatted bibtex files, despite they claiming
  to do so. You will have to clean them up and we recommend to do it
...
we are sure you can automate them ... as you likely do something
wrong or have not thought about cloud computing where we tend to
automatize most of the steps.
...
- Write a report that includes typically the following sections
...
if you have difficulties, but its good to copy and paste from a
working example. Dependent on class your format may be slightly
