...
When following the API-specification-first approach to defining and
developing a RESTful service, the first and foremost step is to define
the API conforming to the OpenAPI specification, and then using
codegen tools to conveniently generate server-side stub code, client code, documentation, in the language you desire. In Section
...
retrieve the virtual cluster (VC) object from the server.
...
and document a RESTful service API. It is also known under version 3.0 of swagger. However, as the tools for 3.0 are not yet completed, we
continue for now to use version swagger 2.0, until the transition
has been completed. This is especially of importance, as we need to
use the swagger codegen tool, which currently supports only up to
...
*info*, and others. Among these fields, ***swagger***, ***info***, and
...
:   defines the actual endpoints of the exposed RESTful API service.
    ...
    */vc/{id}* as the two service endpoints. They will be part of the final service URL, appended after the service *host* and *basePath*,
    which is explained later.

    Let us focus on the *Path Item Object*. It contains one or more supported *operations* on the service endpoint. An *operation* is
    ...
    *Operation Object* that describes the operations in more detail.
    ...
    e.g., **200** as successful return; **40X** as authentication and authorization related errors; and **50x** as other server-side
    servers. It can also have a default response keyed by **default**
    ...
    *consumes* and *produces* to indicate what MIME types it expects
    ...
    the URL path which has the *path* value. When the *in* has *path*
    ...
    required, in most cases, we also use other optional fields.

host

:   to indicate where the service is to be deployed, which could be
    *localhost* or a valid IP address or a DNS name of the host where the service is to be deployed. If another port number other than *80*
...
:   to specify the common base URL to be appended after the *host* to form
...
:   as used in in the *paths* field, in order to point to a customized
    ...
    The *definitions* field contains the object definition of
    ...
    REF link at the top of the document), but the most frequently used
    ones are:
...
:   has the detailed information of each attribute/property of the object, e.g., *type*, *format*. It also supports hierarchical object
...
:   A virtual cluster, which has one Front-End (FE) management node and
    ...
    identify the VC, and *nnodes* to indicate how many compute nodes it has.

FE

:   A management node from which to access the compute nodes. The FE
    node usually connects to all the compute nodes via a private network.

Node

:   A computer node object that the info *ncores* to indicate the number of
